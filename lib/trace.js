...
class Trace {
  ...
  _extractMetadata() {
    ...
    return {
      ...
      monotonicTime: contextEvent?.monotonicTime,
    };
  }
  ...
  getTotalDuration() {
    // Calculate total duration as max(endTime) - min(startTime) to avoid double-counting nested actions
    if (this.actions.length === 0) return 0;
    const startTime = Math.min(...this.actions.map((a) => a.startTime));
    const endTime = Math.max(...this.actions.map((a) => a.endTime));
