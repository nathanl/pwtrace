...
function listScreenshotsForStep(trace, stepNum, format) {
  ...
  const screenshotInfo = screenshots.map((s, idx) => {
    // Screenshot timestamps from filenames are wall times (epoch ms)
    // Convert to monotonic time: (wall - trace.wallTime) + trace.monotonicTime
    const relTime =
      s.timestamp - trace.metadata.wallTime + trace.metadata.monotonicTime;
