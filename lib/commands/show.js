...
function formatSource(sourceLocation) {
  if (!sourceLocation) return "(Not captured)";
  const { file, line } = sourceLocation;
  // Extract basename or last 2-3 path segments for brevity
...
async function showCommand(tracefile, options) {
  try {
    ...
    if (options.format === "json") {
      const output = {
        ...
        actions: trace.actions.map((action, i) => ({
          ...
          source: action.sourceLocation || null,
          error: action.error
            ? safe(
    ...
    const rows = trace.actions.map((action, i) => {
      ...
      const source = formatSource(action.sourceLocation);
      ...
      return hasStepTitles
        ? [num, status, method, target, duration, stepTitle, source, error]
        : [num, status, method, target, duration, source, error];
    });
