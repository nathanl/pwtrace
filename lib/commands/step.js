...
async function stepCommand(tracefile, stepNumber, options) {
  try {
    ...
    // Always show Source line
    if (action.sourceLocation) {
      const { file, line, column } = action.sourceLocation;
      console.log(`Source: ${file}:${line}`);
    } else {
      console.log(`Source: (Not captured)`);
    }
    ...
    const errorLabel =
      ...
      (typeof action.error === "string"
        ...
        ? " (timeout)"
        : "";
    console.log(`Status:   ${action.status.toUpperCase()}${errorLabel}`);
